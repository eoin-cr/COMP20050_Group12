...
public class TileBot {
	...
	private int[] constructiveGrowMinCorridorStrat(List<HabitatTile> deckTiles, Player player) {
		ArrayList<Habitat> minToMaxCorridors = findMinToMaxHabitatCorridors(
				player.getLongestCorridorSizes());
		return matchPreferences(deckTiles, minToMaxCorridors);
	}
	...
	private int[] destructiveTileStrategy(List<HabitatTile> deckTiles, Player nextPlayer) {
		ArrayList<Habitat> nextPlayerMinToMaxCorridors = findMinToMaxHabitatCorridors(
				nextPlayer.getLongestCorridorSizes());
		return matchPreferences(deckTiles, nextPlayerMinToMaxCorridors);
	}

	//helper function, finds a player's smallest to largest habitat corridors,
	// returns a list of habitats in order
	...
	//helper function, matches deckTile preferences from 4 (highest) to 1 (lowest) to the
	// ordered list of habitats given
	private int[] matchPreferences(List<HabitatTile> deckTiles,
								   ArrayList<Habitat> matchHabitatCorridors) {
		//each decktile index has a preference rating attached to it, 4 being highest preference
		// and 1 being lowest
		int[] preferences = new int[4];
		int pref = 4;
		...
		for (Habitat hab : matchHabitatCorridors) {
			for (int i = 0; i < deckTiles.size(); i++) {
				if (deckTiles.get(i).getHabitat1() == hab
						|| deckTiles.get(i).getHabitat2() == hab) {
					preferences[i] = pref;
					pref--;
					break;
				}
...
//
//		if (player.getLongestCorridorSize(tile.getHabitat1()) < player.getLongestCorridorSize(
//				tile.getHabitat2())) {
